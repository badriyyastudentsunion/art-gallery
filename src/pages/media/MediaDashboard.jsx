...
export default function MediaDashboard() {
  ...
  // Handle image resizing and canvas compression to lightweight Base64
  ...
  const handleSubmit = async (e) => {
    ...
    if (mediaType === 'photo' && !imagePreview) {
      showToast('Please select or capture a photo.', 'error')
      return
    }
  ...
  return (
    <div className="med-root">
      ...
      <main className="med-container">
        ...
        <section className="med-card med-form-section">
          ...
          <div className="med-type-tabs">
            {[
              ...
              <button
                ...
                onClick={() => { setMediaType(t.id); setLink(''); setImagePreview(null) }}
              >
                {t.icon}
                <span>{t.label}</span>
              </button>
          ...
          <form onSubmit={handleSubmit} className="med-form">
            <div className="med-field">
              <label className="med-label">Title / Caption</label>
              <textarea
                className="med-input med-textarea"
                rows={2}
                placeholder="Write a suitable caption or title..."
                value={caption}
            ...
            {mediaType === 'photo' ? (
              <div className="med-field">
                <label className="med-label">Photo Upload</label>
                <div className="med-upload-zone">
                  {imagePreview ? (
                    <div className="med-preview-box">
                      <img src={imagePreview} alt="Preview" className="med-image-preview" />
                      <button
                        type="button"
                        className="med-clear-img-btn"
                    ...
                    <label className="med-upload-label">
                      <IconUpload />
                      <span className="med-upload-title">Choose Photo</span>
        ...
        {/* Right Side: Uploaded Feed */}
        <section className="med-card med-feed-section">
          <div className="med-feed-header">
            <h2 className="med-section-title">Live Gallery Feed</h2>
            <span className="med-badge">{mediaFeed.length} items</span>
          </div>

          <div className="med-feed-list">
            {mediaFeed.length === 0 ? (
              <div className="med-feed-empty">
                <IconImage />
                <p>No media updates posted yet.</p>
                <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginTop: 4 }}>Use the form on the left to publish live updates.</p>
              </div>
            ) : (
              mediaFeed.map(item => {
                ...
                return (
                  <div key={item.id} className="med-feed-card">
                    ...
                    <div className="med-feed-info">
                      ...
                      {relatedCompName && (
                        <span className="med-feed-tagged-comp">🏷️ {relatedCompName}</span>
                      )}
                    </div>
