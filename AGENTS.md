...
- When Supabase schema changes (new tables, storage buckets, RLS policies, RPC functions) are required, execute them directly via Supabase MCP `execute_sql` or explicitly state the requirement to the user immediately. Never cut corners or dump data into improper columns due to hesitation or uncommunicated constraints.
