...
- Supabase warning `Value is larger than 10,240 characters` is a direct result of improper data dumping and is strictly forbidden.
...
- When Supabase schema changes (new tables, storage buckets, RLS policies, RPC functions) are required, execute them directly via Supabase MCP `execute_sql` or explicitly state the requirement to the user immediately. Never cut corners or dump data into improper columns.
