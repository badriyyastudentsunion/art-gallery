...
const headers = rows[0].map(h => h.replace(/^\uFEFF/, '').replace(/^\uEFBBBF/, '').trim().toLowerCase());
...
rows.slice(1).forEach((row, i) => {
  ...
  if (desc.trim() || dur.trim() || crit.trim()) {
    ...
    noRules++;
    if (noRules <= 10) console.log('NO RULES - row', i+2, '| Name:', name, '| col count:', row.length);
  }
});
console.log('\nTotal has rules:', hasRules, '  No rules:', noRules);
